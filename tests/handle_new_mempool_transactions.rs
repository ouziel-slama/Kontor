...
fn test_empty_initial_state() {
    let mut initial_mempool_txids: IndexSet<Txid> = IndexSet::new();
    ...
    let result =
        handle_new_mempool_transactions(&mut initial_mempool_txids, &mut mempool_cache, txs);
...
fn test_adding_and_removing_transactions() {
    ...
    let mut initial_mempool_txids: IndexSet<Txid> = IndexSet::from([tx1.txid(), tx2.txid()]);
...
fn test_no_changes() {
    let tx1 = MockTransaction::new(1);
    let mut initial_mempool_txids: IndexSet<Txid> = IndexSet::from([tx1.txid()]);
